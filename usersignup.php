...
//signin.php
include '../Waste/connection.php';
include "../Waste/funcions.php";
$data = file_get_contents("php://input");
$datas = json_decode($data);

@$firstname =validate($datas->firstname);
@$lastname =validate($datas->lastname);
...
isset($datas->firstname,$datas->lastname,$datas->number,$datas->email,$datas->password,$datas->cpassword)?:response(false,"invalid parameter","");
...
if(EmailExistsusers_admin($conn, $email) == true ){
    ...
    response(false, 'Email already exists', null);
        ...
        $hashed_password = password_hash($pass, PASSWORD_DEFAULT);
       $insert_user_query = "INSERT INTO users (firstname,lastname, mail,number, password,is_admin)";
       $insert_user_query.= "VALUES('$firstname', '$lastname','$email', '$number', '$hashed_password','user')";
